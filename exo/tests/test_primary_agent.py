...
class TestPrimaryInterfaceAgent:
    ...
    def test_generate_direct_response(self, mock_get_service):
        ...
        with patch('exo.core.system_prompts.get_system_prompt', return_value="You are a helpful assistant.") as mock_get_system_prompt:
            # Generate a response
            ...
            # Check that the appropriate methods were called
            mock_get_service.assert_called_with(ServiceNames.LLM_MANAGER)
            mock_get_system_prompt.assert_called_with("pia")
    ...
    def test_handle_brave_search(self):
    ...
    def test_extract_path_from_query(self):
        ...
        path = agent._extract_path_from_query("read /home/user/test.txt")
        assert "/" in path and "test.txt" in path

        path = agent._extract_path_from_query("check the file in src/main/app.py")
        assert "/" in path and "app.py" in path
    ...
    def test_list_files(self):
        ...
        # Add the _list_files method to the agent
        agent._list_files = lambda path, mcp_manager: f"Contents of '{path}':\n\n📄 test.txt (100 bytes)\n📁 docs"
    ...
    def test_read_file(self):
        ...
        # Add the _read_file method to the agent
        agent._read_file = lambda path, mcp_manager: f"Contents of '{path}':\n\n```\nThis is the content of the file.\n```"
    ...
    def test_delete_file(self):
        ...
        # Add the _delete_file method to the agent
        agent._delete_file = lambda path, mcp_manager: f"Successfully deleted '{path}'."
